...
**Voice selection** - defaults to "George - Warm, Captivating Storyteller" (`JBFqnCBsd6RMkjVDRZzb`). `--voice` does a name search (partial match, case-insensitive), `--voice-id` uses an exact ElevenLabs voice ID if you know it. `--list-voices` dumps all voices with their IDs.
